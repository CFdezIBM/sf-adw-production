{#-
    ...
    El +schema de dbt_project.yml es el DOMINIO (PRODUCTION). La regla es:

      Studio (IDE)        ->  CARLOS_FERNANDEZ_PRODUCTION   sandbox personal
      Job de CI           ->  PRODUCTION                  en ADW_*_DEV
      Job de UAT / PROD   ->  PRODUCTION                  en ADW_*_UAT / _PROD
    ...
    prefijo, que es el lado seguro: nunca sobreescribe el PRODUCTION compartido.
...
{% macro generate_schema_name(custom_schema_name, node) -%}
    ...
    {%- else -%}
        {%- set resultado = (target.schema | trim | upper) -%}
        {%- set motivo = 'Studio: schema personal' -%}
    {%- endif -%}
